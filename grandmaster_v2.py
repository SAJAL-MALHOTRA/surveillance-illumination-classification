"""
Grandmaster v2: Clean 10-Fold Bagged Pipeline
Building directly on the 0.189 LB winning pipeline (run_aligned_10fold_grandmaster.py):
1. Adds ResNet34 embeddings (cached 512-D) to ResNet18 + EfficientNet-B0 + Handcrafted
2. Multi-seed FastMLP (seeds 42 and 123) to stabilize the dominant 50% weight model
3. Adds tuned LR C=0.005 alongside C=0.01 and C=0.02
4. ZERO pseudo-labeling (prevents confirmation bias / test distribution distortion)
5. SLSQP optimization on Out-Of-Fold predictions
6. Class-balance preserving argmax for optimal test accuracy
"""
...
# 1. Load Features
print("\n[1/4] Loading and Scaling Features...", flush=True)
...
print("\n[4/4] Generating Final Submission on Aligned Test Images...", flush=True)
